...
- fixtures: fixed `choice.exe /T 30 /D Y` timeout mode, fixed `findstr.exe` stdout mode, and fixed `findstr.exe` stderr mode are selected only by provider-private qualification code and validated against exact executable/argument tuples;
